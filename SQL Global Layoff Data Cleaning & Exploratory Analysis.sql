...
-- -------------------------------------------- This is a SQL data cleaning and exploratory analysis project on global compay layoff data from 2020 March to 2023 March-----------------------------------
...
-- Removing irrelevent rows and columns
...
-- Sorting total lay offs per year in descending order
select year(`date`), sum(total_laid_off)
from staging
group by year(`date`)
order by year(`date`) desc;

-- Sorting total lay offs per business stage
select stage, sum(total_laid_off)
from staging
group by stage
order by sum(total_laid_off) desc;

-- Ovewview of data where percentage lay off is 100% by funds raised
select *
from staging
...
-- Industries with the most number of companies with 100% lay offs
...
-- Companies with the most lay offs  
...
-- Industries with the most lay offs and the number of recorded companies
-- * Consumer, Retail, and Transportation has the most total layoffs.
-- * Finance ranked 4th but had 2x the number of comapnies on record.
select industry, sum(total_laid_off), count(company)
from staging
group by industry
order by sum(total_laid_off) desc;

-- Industries with the highest % layoffs
-- * Aerospace had the highest % layoffs of 57% followed by Education 36%.
select round(avg(percentage_laid_off),2) as avg_perc, industry
from staging
group by industry
order by avg_perc desc;

-- Countries with the most lay offs and recorded companies
-- * almost 90% of records were companies from the U.S 
select country, sum(total_laid_off), count(company)
from staging
group by country
order by sum(total_laid_off) desc;

-- Ranking the top 5 companies with the most total lay offs for each year (2020-2023)
...
select country, round(avg(percentage_laid_off),2), count(company)
from staging
group by country
order by count(company) desc;
